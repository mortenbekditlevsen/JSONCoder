...
public protocol TestKeyedDecodingContainerProtocol {
...
open class JSONEncoder {
    ...
    @available(*, deprecated, message: "Please use KeyCodingStrategy instead")
...
private struct _JSONKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case (.useDefaultKeys, .useDefaultKeys):
            return key
        case (.useSnakeCase, _), (.useDefaultKeys, .convertToSnakeCase):
            let newKeyString = JSONEncoder.KeyCodingStrategy._convertToSnakeCase(key.stringValue)
            return _JSONKey(stringValue: newKeyString, intValue: key.intValue)
        case (.custom(let converter), _), (.useDefaultKeys, .custom(let converter)):
            // Note: The codingPath contains a path of unencoded keys
...
open class JSONDecoder {
    ...
    /// The strategy to use for coding keys. Defaults to `.useDefaultKeys`.
    open var keyCodingStrategy: JSONEncoder.KeyCodingStrategy = .useDefaultKeys
    ...
    fileprivate struct _Options {
        ...
        let keyCodingStrategy: JSONEncoder.KeyCodingStrategy
        let userInfo: [CodingUserInfoKey : Any]
    }
...
private struct JSONDecoderCodingKey: CodingKey {
...
private struct _JSONKeyedDecodingContainer<K : CodingKey> : KeyedDecodingContainerProtocol, TestKeyedDecodingContainerProtocol {
    ...
    init(referencing decoder: __JSONDecoder, wrapping container: [String : Any]) {
        ...
        // If the keyCodingStrategy is not .useDefaultKeys then we postpone the conversion
        guard case .useDefaultKeys = decoder.options.keyCodingStrategy else {
            self.container = container
            return
        }
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case .custom(let converter):
            // Note: The codingPath contains a path of unencoded keys
            var pathForKey = self.codingPath
            pathForKey.append(key)
            return converter(pathForKey)
        }
    }
    ...
    public var allRawKeys: [CodingKey] {
        return self.container.keys.map(JSONDecoderCodingKey.init(stringValue:))
    }

    public func contains(_ key: Key) -> Bool {
        return self.container[_converted(key).stringValue] != nil
    }

    @inline(__always) private func getValue<LocalKey: CodingKey>(forKey key: LocalKey) throws -> Any {
        guard let value = container[_converted(key).stringValue] else {
